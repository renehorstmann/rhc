...
static int rhc_log_to_str_v(char *str, rhcsize n,
    ...
    rhcsize format_size = vsnprintf(str + size, n - size, format, vl);
...
void rhc_log_base(enum rhc_log_level level, const char *opt_file, int line,
    ...
    char time_str[16];
    rhcsize time_size = strftime(time_str, sizeof time_str, "%H:%M:%S", lt);
